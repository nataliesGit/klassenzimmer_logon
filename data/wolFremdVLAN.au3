...
Global $MACAddress
Global $broadcast
;~ Global $folder = "C:\Temp2"
;~ if not(FileExists($folder)) Then
;~ 	DirCreate ($folder)
;~ EndIf
;~ Global $datum = _NowDate ( )
;~ MsgBox($MB_SYSTEMMODAL, "datum",$datum)
;~ Global $zeit = _NowTime(4)
;~ $zeit = StringReplace ( $zeit, ":", ".")
;~ Global $FileName = $folder&"\wol_"&$datum &"-"&$zeit&".txt"
;~ MsgBox($MB_SYSTEMMODAL, "Filename",$FileName)
;~ Global $fileHandle = FileOpen($FileName, $FO_APPEND)
...
;~ 		FileWrite($fileHandle,"Wol ausgefuehrt für MAC: "&$MACAddress&@CRLF)
		sleep(500)
