...
Func beispielNotepad()
	$_Run = "notepad.exe " & "import\beispiel.txt"
...
;~ 	MsgBox(0, @ScriptName, FileGetSize ( $sFileOpenDialog ))
	...
	If $handle_read = -1 Then
		...
		Exit 1
	EndIf
	_SQLite_Exec(-1, "CREATE TABLE raumpc (Rechner,Raum,IP,Status,VLAN,MAC);") ; CREATE a Table
...
;~ 			_SQLite_QuerySingleRow($pcdb, "select field1 from raumpc where field2 = '" & $pc & "';" , $aRow)  Beispiel für sql statement mit variable
			_SQLite_Exec(-1, "INSERT INTO raumpc(Rechner,Raum,IP,Status,VLAN,MAC) VALUES ('" & $zeile[1] & "','" & $zeile[2]& "','" & $zeile[3]& "','" & $zeile[4]& "','" & $zeile[5]& "','" & $zeile[6]& "');") ; INSERT Data
		Else
			$importStatus = 1
